...
import { CheckboxInput, TextInput } from '@/components/ui/Input'
...
const props = defineProps({
    ...
    booleanField: { type: String, default: null },
    booleanLabel: { type: String, default: null },
})
...
function add() {
    ...
    rows.value = [...rows.value, {
        ...
        name: draft.value,
        holder_count: 0,
        ...(props.booleanField ? { [props.booleanField]: false } : {}),
    }]
    draft.value = ''
}
        ...
        <table class="w-full table-fixed text-sm">
            <thead>
                <tr class="border-b border-(--color-table-header-separator) text-left text-(--color-table-header-text)">
                    ...
                    <th v-if="booleanField" class="w-24 py-2 pr-3 font-medium">{{ t(booleanLabel) }}</th>
                    <th class="w-14 py-2" />
                </tr>
            </thead>
            <tbody>
                ...
                >
                    ...
                    <td v-if="booleanField" class="py-2 pr-3 align-middle">
                        <CheckboxInput v-model="item[booleanField]" />
                    </td>
                ...
                <tr v-if="!rows.length">
                    <td :colspan="booleanField ? 4 : 3" class="py-6 text-center text-(--color-text-secondary)">
                        {{ t('list_empty') }}
                    </td>
                </tr>

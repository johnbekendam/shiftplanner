...
describe("Settings/Index", () => {
    it("shows a tab for competences, business lines, shifts, information, questions and general", () => {
        ...
        expect(text).not.toContain("Workcenters");
    ...
    it("does not show the Workcenters settings panel", () => {
        const w = mountPage({
            workcenters: [{ id: 3, name: "Line 1", position: 1, archived_at: null, shifts: [] }],
        });

        expect(w.find('[data-testid="panel-workcenters"]').exists()).toBe(false);
        expect(w.text()).not.toContain("Workcenters");
    });

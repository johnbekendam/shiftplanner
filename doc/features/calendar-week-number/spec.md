...
- **Week number column:** a new first column with the ISO week number of each week row, in a subtle color. The weekday header row has a `WK` label above the numbers, in the same color.
- **Selected week:** one rounded border around the whole week row, that is the week number and all 7 day cells. The per-day border on the days of the selected week is removed. The days have no hover border. The week row has the hover border: it shows on any week row that is not selected. When no week is selected (a weekday header is selected instead), no border shows.
...
- **The week row is the selection area.** The week number and the space in the row select the week, and the row has the hover border. They use the first selectable day of the row. A click on a day still selects that day.
...
- A keyboard shortcut for the week row. The day buttons stay the keyboard way to select a week.

...
- [ ] 2. Live data endpoint. `GET /live/{token}` returns the Inertia page `Live` with two week blocks (current and next ISO week). Each block has the shifts of the workcenter, the days Monday to Friday, the assigned names, the open-spot count, and a `published` flag. An unpublished week has no assignments in the props. An unknown token and an archived workcenter return 404. The response has `noindex` and no-cache headers. The route needs no login.

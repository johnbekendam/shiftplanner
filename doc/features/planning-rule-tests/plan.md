...
- [ ] 2. `EligibilityTest`: holiday, unavailable and missing availability, `not_preferred` still assignable, workcenter hard-mode restriction, and shift overlap.
- [ ] 3. Hard `not_preferred_shift`: `PlanEligibility` excludes `not_preferred` cells. Update the `planning-rules` and `scheduling-engine` specs.
